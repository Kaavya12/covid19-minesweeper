document.addEventListener('DOMContentLoaded', ()=>{
	...
	window.addEventListener('resize', ()=>{
		...
		cell.style.width = (boardWidth - 20)/10
    	cell.style.height = (boardWidth - 20)/10
	})
...
let loss = function()
{
	...
	for( let i = 0; i < cells.length; i++ )
	{
		if(board[cells[i]].mined && !board[cells[i]].flagged )
		{
		    ...
		    cellElement.innerHTML = MINE;
		    cellElement.style.color = "black";
		    affected.push(parseInt(board[cells[i]].id))
		    console.log(cellElement)
		}
	}

	
	timeAffect = setTimeout(()=>{
		affectTimer = setInterval(function(){
			toBeAffected = Math.floor(Math.random() * 100);
			
			if (!affected.includes(parseInt(board[cells[toBeAffected]].id))){
				if (!board[cells[toBeAffected]].flagged){
				var cellAffected = document.getElementById(board[cells[toBeAffected]].id);
			    	
						//console.log(cellAffected)
						cellAffected.innerHTML = MINE;
				    	cellAffected.style.color = "black";
				    	affected.push(parseInt(board[cells[toBeAffected]].id))
					...
					//console.log(board[cells[toBeAffected]])
			...
			function stop() {
		        if (affectTimer) {
		            clearInterval(affectTimer);
		            clearTimeout(timeAffect)
		            affectTimer = 0;
		        }
			}	
...
let newGame = function( boardSize, mines )
{
    ...
    messageBoxElement.style.color = "black";
    messageBoxElement.style.backgroundColor = 'white';
...
var FLAG = '<img src="https://lh3.googleusercontent.com/mGD1F4GEnGYv4CAcfHhfYPEMDMfcwDEWF_8eh5J8CrqCWnqqWlNf8JhqUMTBoJ7FGybWTnMI3b2lBilzYQ2l8qKi4ZZENfXnWhUxlQy-kPdKz2MqGdVeiTu1OzUCRadSVmXa7TxnKw" width=95%></img>';
var MINE = '<img src="https://lh3.googleusercontent.com/s_KdsXOxSEN9a-ykuxZKtf6-5boNx-areyDmFJw2JzELr-j_JVxgO-ftW6uFX3qyI5Io2J-PHOYU9AeKrfEr70w2D7IHdddjOVdIEAGtspH2PNRNYq8vAr1dzVUr48TGCK-zYlAdBQ" width=95%></img>';
let boardSize = 10;
let mines = 10;
let timer = 0;
let timeout;
